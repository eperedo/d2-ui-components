...
The visibility of the float action button depends on the onButtonClick prop, if it has a function defined it will be visible, if it is undefined it will be hidden.
...
`^(?!${escape( path.normalize(appSrc + '/').replace(/[\\]+/g, '/') )}).+/node_modules/(?!d2-ui-components)`,
'g'
);
console.log({regexp})
return regexp;
